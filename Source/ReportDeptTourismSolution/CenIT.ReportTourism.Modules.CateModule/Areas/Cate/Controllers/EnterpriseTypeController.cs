...
{
    public class EnterpriseTypeController : AppController
    {
        private readonly CateEnterpriseTypeCache _cache = new CateEnterpriseTypeCache();
        private readonly string _title = AppProcessor.Messagor.GetMessage("EnterpriseType_Title");
        ...
        [ActionType(Type = EnumActionType.Add)]
        [ValidateInput(false)]
        public ActionResult Add(CateEnterpriseTypeModel model)
        {
            if (!ModelState.IsValid)
                return PartialView("_EnterpriseType", model);

            var id = _cache.Save(new CateEnterpriseTypeModel
            {
        ...
        [ActionType(Type = EnumActionType.Edit)]
        [ValidateInput(false)]
        public ActionResult Edit(CateEnterpriseTypeModel model)
        {
            if (!ModelState.IsValid)
                return PartialView("_EnterpriseType", model);
        ...
        [HttpPost]
        [ActionType(Type = EnumActionType.Delete)]
        public ActionResult Delete(CateEnterpriseTypeModel model)
        {
            var delete = _cache.GetById(model.EnterpriseTypeId);
