...
{
    public class EnterpriseStatusController : AppController
    {
        private readonly CateEnterpriseStatusCache _cache = new CateEnterpriseStatusCache();
        private readonly string _title = AppProcessor.Messagor.GetMessage("EnterpriseStatus_Title");
        ...
        [ActionType(Type = EnumActionType.Add)]
        [ValidateInput(false)]
        public ActionResult Add(CateEnterpriseStatusModel model)
        {
            if (!ModelState.IsValid)
                return PartialView("_EnterpriseStatus", model);

            var id = _cache.Save(new CateEnterpriseStatusModel
            {
        ...
        [ActionType(Type = EnumActionType.Edit)]
        [ValidateInput(false)]
        public ActionResult Edit(CateEnterpriseStatusModel model)
        {
            if (!ModelState.IsValid)
                return PartialView("_EnterpriseStatus", model);
        ...
        [HttpPost]
        [ActionType(Type = EnumActionType.Delete)]
        public ActionResult Delete(CateEnterpriseStatusModel model)
        {
            var delete = _cache.GetById(model.EnterpriseStatusId);
