...
{
    public class TypeTravelingController : AppController
    {
        ...
        public TypeTravelingController()
        {
        ...
        public ActionResult Info(int enterpriseId)
        {
            ...
            return PartialView("_Info", travelingServiceModel);
        }
        ...
        public ActionResult SaveCertificate(CateTravelingServiceCertificateModel model)
        {
            ...
            {
                var certificateModel = _travelingServiceCache.GetCertificateViaEnterprise(model.EnterpriseId);
                certificateModel = certificateModel ?? new CateTravelingServiceCertificateModel();
            ...
            var dataLicenseCertificate =
                CreateDataRefDocs(model.EnterpriseId, model.FileLicenseFile, _travelingCertificateTravelEscrowFolder,
                    out fileId);
        ...
        public ActionResult CertificateTravelEscrow(int enterpriseId)
        {
            ...
            certificateTravelEscrowModel.AnnouncedView =
        ...
        public ActionResult SaveCertificateTravelEscrow(CateTravelingServiceCertificateTravelEscrowModel model)
        {
            if (!ModelState.IsValid)
            {
                var cul = CultureInfo.GetCultureInfo("vi-VN");
                var certificateTravelEscrowModel =
                    _travelingServiceCache.GetCertificateViaEnterpriseTravelEscrow(model.EnterpriseId);
                certificateTravelEscrowModel = certificateTravelEscrowModel ??
                                               new CateTravelingServiceCertificateTravelEscrowModel();
                certificateTravelEscrowModel.EnterpriseId = model.EnterpriseId;
                certificateTravelEscrowModel.AnnouncedView =
                    certificateTravelEscrowModel.Amount.ToString("#,### VND", cul.NumberFormat);
                return PartialView("_TabCertificateTravelEscrow", certificateTravelEscrowModel);
            }
            ...
            var dataCertificate =
                CreateDataRefDocs(model.EnterpriseId, model.FileCertificate, _travelingCertificateTravelEscrowFolder,
                    out fileId);

            model.CertificateId = fileId == Guid.Empty ? model.CertificateId : fileId;
            ...
            if (enterpriseId > 0 && model.FileCertificate != null)
            {
                ...
                if (!Directory.Exists(moduleRefDocsAbsolutePathFolder))
                    Directory.CreateDirectory(moduleRefDocsAbsolutePathFolder);
                var sFileSecurityCertificatePath = string.Concat(moduleRefDocsAbsolutePathFolder, "/",
                    model.CertificateId.ToString().ToUpper(), Path.GetExtension(model.FileCertificate.FileName));
