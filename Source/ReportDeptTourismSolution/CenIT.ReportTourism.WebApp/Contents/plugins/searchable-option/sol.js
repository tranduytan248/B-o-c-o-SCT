...
(function($, window, document) {
    ...
    SearchableOptionList.prototype = {
        ...
        defaults: {
            data: undefined,
            name:
                undefined, // name attribute, can also be set as name="" attribute on original element or data-sol-name=""
            ...
            events: {
                ...
                onScroll: function() {
                    ...
                    var selectionContainerYPos = this.$input.offset().top -
                            this.config.scrollTarget.scrollTop() +
                            this.$input.outerHeight(false),
                        ...
                        displayContainerAboveInput = this.config.displayContainerAboveInput ||
                            document.documentElement.clientHeight - this.config.scrollTarget.scrollTop() <
                    ...
                    if (displayContainerAboveInput) {
                        ...
                        selectionContainerYPos = this.$input.offset().top -
                            selectionContainerHeight -
                            this.config.scrollTarget.scrollTop() +
                            parseInt(this.$selectionContainer.css("border-bottom-width"), 10);
                        this.$container
                    ...
                    if (this.$innerContainer.css("display") !== "block") {
                        ...
                        var borderRadiusSelector = displayContainerAboveInput
                    ...
                    //    .css('left', Math.floor(this.$innerContainer.outerWidth(false)))
                    //    .css('width', selectionContainerWidth);

                    this.$selectionContainer
                        .css("margin-top", this.$innerContainer.outerHeight(false))
                        //.css('left', Math.floor(this.$innerContainer.outerWidth(false)))
                        .css("width", this.$innerContainer.outerWidth(false));
            ...
            showSelectAll: function() {
                return this.config.multiple &&
                    this.config.selectAllMaxItemsThreshold &&
                    this.items &&
                    this.items.length <= this.config.selectAllMaxItemsThreshold;
            },
        ...
        _initializeUiElements: function() {
            ...
            this.$input = $('<input type="text"/>')
                .addClass(this.config.inputClass)
            ...
            var $inputContainer = $('<div class="sol-input-container"/>')
                .append(this.$input);
            this.$innerContainer = $('<div class="sol-inner-container"/>')
                .addClass(this.config.inputClass)
                .append($inputContainer).append(this.$caret);
                    ...
                    function(e) {
                        ...
                        if (!self.$noResultsItem.is(":visible")) {
                            ...
                            if (keyCode === 40 || keyCode === 38) {
                                ...
                                var indexOfNextHighlightedOption = $allVisibleOptions.index($currentHighlightedOption) +
                                    directionValue;
                                ...
                                self.$selection.scrollTop(self.$selection.scrollTop() +
                                    $nextHighlightedOption.position().top);

                                preventDefault = true;
                            } else if (self.keyboardNavigationMode === true && keyCode === 32) {
                                // toggle current selected item with space bar
                                $currentHighlightedOption =
                                    self.$selection.find(".sol-option.keyboard-selection input");
                                $currentHighlightedOption
        ...
        _syslySearchTermFilter: function() {
        ...
        _findTerms: function(dataArray, searchTerm) {
            ...
            this._setNoResultsItemVisible(
                this.$selectionContainer.find(".sol-option:not(.sol-filtered-search)").length === 0);
        },
        ...
        _detectDataFromOriginalElement: function() {
            if (this.$originalElement.prop("tagName").toLowerCase() === "select") {
                ...
                $.each(this.$originalElement.children(),
                    function(index, item) {
                        var $item = $(item),
                        ...
                        if (itemTagName === "option") {
                            ...
                            self._showErrorLabel("Invalid element found in select: " +
                                itemTagName +
                                ". Only option and optgroup are allowed");
                        }
                    });
                ...
                this._showErrorLabel(
                    'Could not determine data from original element. Must be a select or data must be provided as data-sol-data="" attribute');
            }
        },

        _processSelectOption: function($option) {
            return $.extend({},
                this.SOL_OPTION_FORMAT,
                {
                    value: $option.val(),
        ...
        _processSelectOptgroup: function($optgroup) {
            var self = this,
                solOptiongroup = $.extend({},
                    this.SOL_OPTIONGROUP_FORMAT,
                    {
                ...
                $inputElement = $('<input type="radio" class="sol-radio"/>')
                    .on("change",
                        function() {
                            ...
                            self.$selectionContainer.find('input[type="radio"][name="' + inputName + '"]').not($(this))
        ...
        _initializeSelectAll: function() {
            ...
            if (this.config.showSelectAll === true ||
                ($.isFunction(this.config.showSelectAll) && this.config.showSelectAll.call(this))) {
                // buttons for (de-)select all
                var self = this,
                    $deselectAllButton = $('<a href="#" class="sol-deselect-all"/>').html(this.config.texts.selectNone)
                        .click(function(e) {
                            self.deselectAll();
                            e.preventDefault();
                            return false;
                        }),
                    $selectAllButton = $('<a href="#" class="sol-select-all"/>').html(this.config.texts.selectAll)
                        .click(function(e) {
                            self.selectAll();
                            e.preventDefault();
                            return false;
                ...
                this.$actionButtons = $('<div class="sol-action-buttons"/>').append($selectAllButton)
                    .append($deselectAllButton).append('<div class="sol-clearfix"/>');
        ...
        selectAll: function() {
            if (this.config.multiple) {
                var $changedInputs = this.$selectionContainer
                    .find(
                        'div.sol-optiongroup:not(.sol-filtered-search) input[type="checkbox"]:not([disabled], :checked)')
                    .prop("checked", true)
                    .trigger("change", true);
        ...
        invert: function() {
            if (this.config.multiple) {
                ...
                var $openedInputs = this.$selectionContainer
                    .find('div.sol-optiongroup:not(.sol-filtered-search) input[type="checkbox"]')
